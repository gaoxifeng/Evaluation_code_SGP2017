...
{
  return abs((p0-p3).dot((p1-p3).cross(p2-p3)))/6.0f;
}
...
scalar ComputeWeightInterface::fractionCell2DBF(const scalar& v0,const scalar& v1,const scalar& v2,const scalar& v3)
{
  ...
  INFOV("Sum: %f",convert<double>()(sum))
  switch(tag) {
  case 0:
    return 0.0f;
  case 1:
...
void ComputeWeightInterface::debugFractionTet()
{
  ...
  for(sizeType i=0; i<16; i++) {
    ...
    INFOV("Fraction Tet %ld:",i)
    INFOV("\tBruteForce: %f,%f,%f,%f",
          convert<double>()(fracFBA),
          convert<double>()(fracFBB),
          convert<double>()(fracFBC),
          convert<double>()(fracFBD))
    INFOV("\tFastCalcul: %f,%f,%f,%f",
          convert<double>()(fracA),
          convert<double>()(fracB),
          convert<double>()(fracC),
          convert<double>()(fracD))

    ASSERT(abs(fracFBA-fracFBB) < EPS)
    ASSERT(abs(fracFBA-fracFBC) < EPS)
    ASSERT(abs(fracFBA-fracFBD) < EPS)

    ASSERT(abs(fracFBA-fracA) < EPS)
    ASSERT(abs(fracFBA-fracB) < EPS)
    ASSERT(abs(fracFBA-fracD) < EPS)
    ASSERT(abs(fracFBA-fracC) < EPS)
  }
}
void ComputeWeightInterface::debugFractionCell2D()
{
  ...
  for(sizeType i=0; i<16; i++) {
    ...
    INFOV("Fraction Cell 2D: %f %f",convert<double>()(fracA),convert<double>()(fracB))
    ASSERT(abs(fracA-fracB) < EPS)
  }
}
void ComputeWeightInterface::debugFractionCell3D()
{
  ...
  INFOV("FractionCell3DFastCalcul: %f",convert<double>()(fractionCell3D(2.0f,2.0f,-2.0f,-2.0f,6.0f,6.0f, 2.0f, 2.0f)))
  INFOV("FractionCell3DBruteForce: %f",convert<double>()(fractionCell3DBF(2.0f,2.0f,-2.0f,-2.0f,6.0f,6.0f, 2.0f, 2.0f, v0,v1,v2,v3,v4,v5,v6,v7)))
}
