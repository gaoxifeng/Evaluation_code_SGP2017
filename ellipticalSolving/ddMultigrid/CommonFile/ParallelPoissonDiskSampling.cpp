  ...
  for(sizeType x=0; x<nrPoint.x(); x++)
    for(sizeType y=0; y<nrPoint.y(); y++)
      for(sizeType z=0; z<nrPoint.z(); z++) {
        sizeType off=abs(RandHash::randhashI(seed++))%sizeType(phaseVals.size());
...
void ParallelPoissonDiskSampling::generateRawSample(ObjMeshTpl<scalar>& mesh)
{
  ...
  _PSet.resize(convert<sizeType>()(_density*poss[nrT]/singleArea));
...
void ParallelPoissonDiskSampling::generateRawSample3D(const ScalarField& phi,bool vol)
{
  ...
  sizeType nrPCell=convert<sizeType>()(ceil(scalar(ratio*_density)));
  ...
  if(_dim == 2) {
    for(sizeType x=-2; x<=2; x++)
      for(sizeType y=-2; y<=2; y++) {
        Vec3i id(x,y,0);
        if(id != Vec3i::Zero()) {
          Vec3 minDist=Vec3(std::max<scalar>(scalar(abs(x))-1.0f,0.0f),
                            std::max<scalar>(scalar(abs(y))-1.0f,0.0f),0.0f)*_cellSz;
    ...
    for(sizeType x=-2; x<=2; x++)
      for(sizeType y=-2; y<=2; y++)
        for(sizeType z=-2; z<=2; z++) {
          ...
          if(id != Vec3i::Zero()) {
            Vec3 minDist=Vec3(std::max<scalar>(scalar(abs(x))-1.0f,0.0f),
                              std::max<scalar>(scalar(abs(y))-1.0f,0.0f),
                              std::max<scalar>(scalar(abs(z))-1.0f,0.0f))*_cellSz;
            if(minDist.norm() >= _radius)
              continue;
            neighs.push_back(id.dot(_stride));
            nrNeigh++;
