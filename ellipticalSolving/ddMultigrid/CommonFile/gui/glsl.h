...
public:
  ...
  bool       link(GLsizei count=0,const char** varyings=NULL,GLenum mode=GL_SEPARATE_ATTRIBS);                        //!< Link all Shaders
...
protected:
  ...
  void        UsesGeometryShader(bool bYesNo) {
    _bUsesGeometryShader = bYesNo;
  }
...
public:
  ...
  glShader* loadfromFile(const char* vertexFile, const char* fragmentFile,GLsizei count=0,const char** varyings=NULL,GLenum mode=GL_SEPARATE_ATTRIBS);    //!< load vertex/fragment shader from file. If you specify 0 for one of the shaders, the fixed function pipeline is used for that part. \param vertexFile Vertex Shader File. \param fragmentFile Fragment Shader File.
  glShader* loadfromMemory(const char* vertexMem, const char* fragmentMem,GLsizei count=0,const char** varyings=NULL,GLenum mode=GL_SEPARATE_ATTRIBS); //!< load vertex/fragment shader from memory. If you specify 0 for one of the shaders, the fixed function pipeline is used for that part.

  // With Geometry Shader (Vertex+Geomentry+Fragment Shader)
  glShader* loadfromFile(const char* vertexFile, const char* geometryFile, const char* fragmentFile,GLsizei count=0,const char** varyings=NULL,GLenum mode=GL_SEPARATE_ATTRIBS); //!< load vertex/geometry/fragment shader from file. If you specify 0 for one of the shaders, the fixed function pipeline is used for that part. \param vertexFile Vertex Shader File. \param geometryFile Geometry Shader File \param fragmentFile Fragment Shader File.
  glShader* loadfromMemory(const char* vertexMem, const char* geometryMem, const char* fragmentMem,GLsizei count=0,const char** varyings=NULL,GLenum mode=GL_SEPARATE_ATTRIBS); //!< load vertex/geometry/fragment shader from memory. If you specify 0 for one of the shaders, the fixed function pipeline is used for that part.
