...
bool FPSCamera::preDraw(PVSMScene& s,boost::shared_ptr<PVSMScene::PVSMSource> src)
{
  ...
  glLoadIdentity();
  gluLookAt(convert<GLdouble>()(_pos[0]),convert<GLdouble>()(_pos[1]),convert<GLdouble>()(_pos[2]),
            convert<GLdouble>()(dir[0]), convert<GLdouble>()(dir[1]), convert<GLdouble>()(dir[2]),
            convert<GLdouble>()(_frame.col(2)[0]),
            convert<GLdouble>()(_frame.col(2)[1]),
            convert<GLdouble>()(_frame.col(2)[2]));
...
bool FPSCamera::motion(PVSMScene& s,boost::shared_ptr<PVSMScene::PVSMSource> src,int x,int y)
{
  ...
  if(_scaleMode) {
    ...
    _pos=(_pos-ctr)*exp((y-_lastY)*_scaleSpd)+ctr;
  }
  if(_transMode) {
...
bool FPSCamera::keyboard(PVSMScene& s,boost::shared_ptr<PVSMScene::PVSMSource> src,unsigned char key,int x,int y)
{
  ...
  if(key == 'o') {
    Vec3 dir=calcDir()+_pos;
    INFOV("From: (%f,%f,%f)",_pos.cast<double>()[0],_pos.cast<double>()[1],_pos.cast<double>()[2])
    INFOV("To: (%f,%f,%f)",dir.cast<double>()[0],dir.cast<double>()[1],dir.cast<double>()[2])
  }
  return false;
}
...
void FPSCamera::setDir(const Vec3& dir)
{
  ...
  _theta=atan2(f[1],f[0])*180.0f/M_PI;
  _phi=atan2(f[2],f.block<2,1>(0,0).norm())*180.0f/M_PI;
}
