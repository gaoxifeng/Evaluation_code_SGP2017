...
public:
  ...
  //setter
  DACGSolver(void) {
    setSolverParameters(1e-20f,1e-5f,10000);
  }
  ...
  const Vec& getXK() const {return _xk;}
  //set matrix A
  ...
  virtual void setKrylovA(boost::shared_ptr<KrylovMatrix<T,KERNEL_TYPE> > A) {
    resize(A->n());
    _A=A;
  }
  ...
  virtual void setKrylovB(boost::shared_ptr<KrylovMatrix<T,KERNEL_TYPE> > B) {
    resize(B->n());
    _B=B;
  }
  ...
  virtual void setKrylovU0(boost::shared_ptr<KrylovMatrix<T,KERNEL_TYPE> > U0) {
    _kryU0=U0;
  }
public:
  //solver
  ...
  RESULT solve(sizeType s,Vec& lambda,Eigen::Matrix<T,-1,-1>& U) {
    ...
    //adjust
    {
      ...
      INFOV("User want %ld eigenpairs",s)
      lambda.resize(s);
      U.resize(n,s);
      _iterCount.resize(s);
    }
    ...
    for(sizeType eig=1; eig<=s; eig++) {
      INFOV("Computing the %ldth pair",eig)
      findInit(U,eig);
      if(_cb)_cb->reset();
  ...
  RESULT solve(const Eigen::Matrix<T,-1,-1>& U,sizeType eig,sizeType& iter,T& qk,T& eta) {
    ...
    if(gamma < 0.0f) {
      WARNINGV("A not positive definite because we found x^TAx=%f!",gamma)
...
#define CHEAP_UPDATE_GAMMA_ETA
      ...
      if(qk < 0.0f) {
        if(gamma < 0.0f)
          WARNINGV("A not positive definite because we found x^TAx=%f!",gamma)
        if(eta < 0.0f)
          WARNINGV("B not positive definite because we found x^TBx=%f!",eta)
#ifdef CHEAP_UPDATE_GAMMA_ETA
        return Solver<T,KERNEL_TYPE>::USER_REQUEST;
#else
        exit(EXIT_FAILURE);
  ...
  const vector<sizeType>& getIterationsCount() const {
    return _iterCount;
  }
...
protected:
  ...
  boost::shared_ptr<KrylovMatrix<T,KERNEL_TYPE> > _kryU0;
