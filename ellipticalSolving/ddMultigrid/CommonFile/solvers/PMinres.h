...
public:
  ...
  virtual typename Solver<T,KERNEL_TYPE>::SOLVER_RESULT solve(const Vec& rhs,Vec& x) {
    ...
    if(_debug) {
      INFOV("shift=%f, tol=%f, maxIter=%d",convert<double>()(_shift),convert<double>()(_toleranceFactor),(int)_maxIterations)
    ...
    T d_norm,alfa,dbar,epln,dlta,gbar,gamal2,tau,t2,t1,epsx;//,Arnorm,relArnorm,epsr;
