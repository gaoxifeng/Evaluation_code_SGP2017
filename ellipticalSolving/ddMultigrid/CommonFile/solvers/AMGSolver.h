...
public:
  ...
  sizeType n() const {
    return _levels[0]->_A.row();
  }
  void setAsPreconditioner(bool asPrecon) {
    _asPrecon=asPrecon;
  }
  sizeType nrLv() const {
    return _nrLv;
  }
  void setNrSmooth(sizeType nr) {
    _nrSmooth=nr;
  }
  void setNrSmoothFinal(sizeType nr) {
    _nrSmoothFinal=nr;
  }
...
#define FINE 2
  ...
  virtual AMGLevel* getNewLevel() const {
    return new AMGLevel();
  }
