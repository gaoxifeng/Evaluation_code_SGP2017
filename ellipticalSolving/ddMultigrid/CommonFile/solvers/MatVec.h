...
public:
  ...
  void kroneckerProd(const MAT& B,FixedSparseMatrix& out,const T& eps=0.0f) const {
    ...
    kroneckerProd(B.rows(),B.cols(),trips,out);
  }
  template <typename ST>
  void kroneckerProd(sizeType R,sizeType C,const std::vector<Eigen::Triplet<T,ST> >& tripsIn,FixedSparseMatrix& out) const {
    ...
    for(sizeType r=0; r<rows(); r++)
      for(sizeType c=_rowStart[r]; c<_rowStart[r+1]; c++)
        for(sizeType i=0; i<nrTIn; i++) {
          ...
          trips[c*nrTIn+i]=Eigen::Triplet<T,ST>((ST)(r*R+TI.row()),(ST)(_value[c].second*C+TI.col()),_value[c].first*TI.value());
        }
