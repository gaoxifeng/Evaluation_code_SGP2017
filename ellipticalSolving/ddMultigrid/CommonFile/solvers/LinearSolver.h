...
public:
  ...
  using Solver<T,KERNEL_TYPE>::_iterationsOut;
  virtual void setMatrix(const FixedSparseMatrix<T,KERNEL_TYPE>& matrix,bool syncPrecon) {
    _diag.resize(matrix.rows());
    OMP_PARALLEL_FOR_
    for(sizeType i=0; i<matrix.rows(); i++)
      _diag[i]=matrix(i,i);
    _diag.array()=1/_diag.array();
  }
  virtual typename Solver<T, KERNEL_TYPE>::SOLVER_RESULT solve(const Vec& rhs,Vec& result) {
...
public:
  ...
  struct MatrixFilter {
    virtual bool operator()(sizeType r,sizeType c) const {
      return false;
    }
  };
public:
  ...
  virtual void setFilter(boost::shared_ptr<MatrixFilter> filter) {
    _filter=filter;
  }
  virtual typename Solver<T,KERNEL_TYPE>::SOLVER_RESULT solve(const Vec& rhs,Vec& result) {
...
public:
  ...
  virtual void setUseIPCG(bool useIPCG) {
    _useIPCG=useIPCG;
  }
  ...
  void setUseOneNorm(bool oneNorm) {
    _oneNorm=oneNorm;
  }
  void setUseWarmStart(bool warmStart) {
  ...
  virtual typename Solver<T,KERNEL_TYPE>::SOLVER_RESULT solve(const Vec& rhs,Vec& result) {
    ...
    while(j <= _maxIterations) {
      ...
      for(i=0; i < _m && j <= _maxIterations; i++,j++) {
        ...
        if((_residualOut=abs(_s(i+1))/normb) < _toleranceFactor) {
