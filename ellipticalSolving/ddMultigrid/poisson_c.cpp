...
void writeMatrix(const DHMLinearSystem::SMAT& lhs,const string& path)
{
...
int main(int argc,char** argv)
{
  ...
  if(argc >= 3) {
    ...
    for(sizeType i=0; i<rhs.size(); i++)
    {
      ...
      {
           const Vec3& v = mesh.getVert(i)._pos;
           double x = v[0];
           double y = v[1];
           double z = v[2];
           rhs[i] = cos(x) + y*z + exp(x + z);
      }
    }
    //rhs.setOnes();
    vector<sizeType> cv;
...
//    std::cout << "Hi" << std::endl;
    ...
    for(sizeType i=0;i<mesh.nrVert();i++)
    {
        if (!mesh.getVert(i)._isSurface)
        {
            sum += (RES[i] - rhs[i]) * (RES[i] - rhs[i]);
        }
    }
    sum = sqrt(sum);

    boost::filesystem::ofstream osc("./e.txt");
    osc << sum << "\n";
    for(sizeType i=0;i<mesh.nrVert();i++)
    {
        if (!mesh.getVert(i)._isSurface)
