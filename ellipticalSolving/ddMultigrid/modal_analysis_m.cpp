...
static T specRad(const FixedSparseMatrix<T,Kernel<T> >& G,Vec* ev,T eps,boost::shared_ptr<KrylovMatrix<T> > kry)
{
  ...
  for(sizeType iter=0;; iter++) {
    ...
    tmpOut/=normTmpOut;
    delta=(tmpOut-tmp).maxCoeff();
    INFOV("Power Iter %ld Err: %1.11f, SpecRad: %f",iter,delta,normTmpOut)
    if(delta <= eps && abs(normTmpOut-lastNormTmpOut) <= eps) {
      if(ev)*ev=tmp;
      return normTmpOut;
    }
    tmp=tmpOut;
    lastNormTmpOut=normTmpOut;
  }
}

int main(int argc,char** argv)//ModalAnalysis
{
  //read mesh
  if(argc < 2) {
...
int mainTestGroundTruth(int argc,char** argv)
{
  //read mesh
  if(argc < 2) {
    INFO("Usage: exe [mesh].vtk [boundary_value_file]")
